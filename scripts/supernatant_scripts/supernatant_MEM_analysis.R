...
# Brydens supernatant results
all <- read.csv("../../data/raw_data/supernatant_data/bes_project_all_no1.csv")

# working on 62 h timepoint data and removing the TY controls
all62 <- all[all$time == 62 & all$supernatant != "100% TY" & all$supernatant != "50% TY",]

# averaging over the means
all62_means <- na.omit(as.data.frame.table(tapply(all62$comp_ind, list(all62$inoc_geno, all62$sup_geno, all62$supernatant, all62$inoculant), mean)))
names(all62_means) <- c("inoc_geno", "sup_geno", "supernatant", "inoculant", "comp_ind")
...
# full model
fit <- lmer(comp_ind ~ inoc_geno * sup_geno + (1|inoculant) + (1|supernatant), REML = F, data=all62_means)
summary(fit)

# removed inoculant random effect
fit_noinoc <- lmer(comp_ind ~ inoc_geno * sup_geno + (1|supernatant), REML = F, data=all62_means)

# removed supernatant random effect
fit_nosup <- lmer(comp_ind ~ inoc_geno * sup_geno + (1|inoculant), REML = F, data=all62_means)
...
fit2 <- lmer(comp_ind ~ inoc_geno * sup_geno + (1|inoculant) + (1|supernatant), REML = F, data=all62_means)
...
fit2_graph_data <- summarySE(data = all62_means, measurevar = 'comp_ind', groupvars = c('inoc_geno', 'sup_geno'))
...
(fit2_graph <- ggplot(data = fit2_graph_data, aes(x = inoc_geno, y = comp_ind,
                      fill = inoc_geno, col = inoc_geno)) +
    geom_jitter(data = all62_means, alpha = 0.2, col = "black", width = 0.1) +
    geom_point(size = 3) +
    geom_errorbar(aes(ymin = comp_ind - ci, ymax = comp_ind + ci), width=0.6, size=1.5) +
    geom_hline(yintercept = 1) +
    facet_wrap(. ~ sup_geno, nrow = 1) +
    labs(x = 'Inoculant genospecies group',
         y = 'Mean Relative Growth Index (RGI)') +
...
ggsave(plot = fit2_graph, '../../data/intermediate_data/supernatant_data/MEMsupernatant_ci_inocsupfacets.pdf', width = 25, height = 15, units = 'cm')



# now run without interaction and do LR test
fit2_noint <- lmer(comp_ind ~ inoc_geno + sup_geno + (1|inoculant) + (1|supernatant), REML = F, data=all62_means)
...
# fit2_noint alleffects as two ggplots
fit2_graph_data_inoc <- summarySE(data = all62_means, measurevar = 'comp_ind', groupvars = c('inoc_geno'))
fit2_graph_data_sup <- summarySE(data = all62_means, measurevar = 'comp_ind', groupvars = c('sup_geno'))

(fit2_graph_inoc <- ggplot(data = fit2_graph_data_inoc, aes(x = inoc_geno, y = comp_ind,
                                                            col = inoc_geno, fill = inoc_geno)) +
    geom_jitter(data = all62_means, alpha = 0.2, col = "black", width = 0.1) +
    geom_point(size = 3) +
    geom_errorbar(aes(ymin = comp_ind - ci, ymax = comp_ind + ci), width=0.6, size=1.5) +
    geom_hline(yintercept = 1) +
...
ggsave(plot = fit2_graph_inoc, '../../data/intermediate_data/supernatant_data/MEMsupernatant_ci_inoc.pdf', width = 12, height = 15, units = 'cm')

(fit2_graph_sup <- ggplot(data = fit2_graph_data_sup, aes(x = sup_geno, y = comp_ind,
                                                          col = sup_geno, fill = sup_geno)) +
    geom_jitter(data = all62_means, alpha = 0.2, col = "black", width = 0.1) +
    geom_point(size = 3) +
    geom_errorbar(aes(ymin = comp_ind - ci, ymax = comp_ind + ci), width=0.6, size=1.5) +
    geom_hline(yintercept = 1) +
    labs(x = 'Supernatant genospecies group',
         y = 'Mean Relative Growth Index (RGI)') +
    scale_colour_manual(values = genoPalette) +
    scale_fill_manual(values = genoPalette) +
    theme_bw()+
...
ggsave(plot = fit2_graph_sup, '../../data/intermediate_data/supernatant_data/MEMsupernatant_ci_sup.pdf', width = 12, height = 15, units = 'cm')
...
fit2_nosupgeno <- lmer(comp_ind ~ inoc_geno + (1|inoculant) + (1|supernatant), REML = F, data=all62_means)
...
# remove the extreme strains 168A and 154C
all62_means2 <- all62_means[all62_means$inoculant != "168A" & all62_means$supernatant != "168A" ,]
all62_means3 <- all62_means2[all62_means2$inoculant != "154C" & all62_means2$supernatant != "154C" ,]


# fit without strains 168 and 154C
fit2_remstrains <- lmer(comp_ind ~ inoc_geno * sup_geno + (1|inoculant) + (1|supernatant), REML = F, data=all62_means3)
...
# 
#      AIC      BIC   logLik deviance df.resid 
#   -934.8   -856.2    486.4   -972.8      443 
# 
# Scaled residuals: 
#     Min      1Q  Median      3Q     Max 
# -4.3904 -0.5580 -0.0188  0.5071  4.1297 
# 
# Random effects:
#  Groups      Name        Variance Std.Dev.
#  inoculant   (Intercept) 0.005460 0.07389 
#  supernatant (Intercept) 0.004472 0.06687 
#  Residual                0.005380 0.07335 
# Number of obs: 462, groups:  inoculant, 22; supernatant, 22
# 
# Fixed effects:
#                         Estimate Std. Error        df t value Pr(>|t|)    
# (Intercept)              0.95298    0.04749  49.78721  20.067  < 2e-16 ***
# inoc_genoOC              0.04985    0.04790  29.84159   1.041  0.30640    
# inoc_genoOE              0.12176    0.05473  29.51277   2.225  0.03389 *  
# inoc_genoCC              0.06182    0.04950  29.75175   1.249  0.22147    
# sup_genoOC              -0.04500    0.04422  31.52945  -1.017  0.31670    
# sup_genoOE              -0.37603    0.05050  31.12300  -7.446 2.13e-08 ***
# sup_genoCC               0.05627    0.04569  31.41836   1.231  0.22728    
# inoc_genoOC:sup_genoOC   0.07326    0.02654 419.05229   2.760  0.00603 ** 
# inoc_genoOE:sup_genoOC   0.02403    0.02973 419.05229   0.808  0.41934    
# inoc_genoCC:sup_genoOC  -0.03337    0.02702 419.05229  -1.235  0.21757    
# inoc_genoOC:sup_genoOE   0.27146    0.02973 419.05229   9.131  < 2e-16 ***
# inoc_genoOE:sup_genoOE   0.32202    0.03543 419.05229   9.089  < 2e-16 ***
# inoc_genoCC:sup_genoOE   0.27555    0.03068 419.05229   8.980  < 2e-16 ***
# inoc_genoOC:sup_genoCC   0.01389    0.02702 419.05229   0.514  0.60742    
# inoc_genoOE:sup_genoCC  -0.04022    0.03068 419.05229  -1.311  0.19061    
# inoc_genoCC:sup_genoCC  -0.03882    0.02841 419.05229  -1.367  0.17246    
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1


# fit without strains 168 and 154C and interaction
fit2_remstrain_noint <- lmer(comp_ind ~ inoc_geno + sup_geno + (1|inoculant) + (1|supernatant), data=all62_means3)

# fit without strains 168 and 154C and sup_geno parameter
fit2_remstrains_nosupgeno <- lmer(comp_ind ~ inoc_geno + (1|inoculant) + (1|supernatant), data=all62_means3)
...
# Data: all62_means3
# Models:
# fit2_remstrains_nosupgeno: comp_ind ~ inoc_geno + (1 | inoculant) + (1 | supernatant)
# fit2_remstrain_noint: comp_ind ~ inoc_geno + sup_geno + (1 | inoculant) + (1 | supernatant)
# fit2_remstrains: comp_ind ~ inoc_geno * sup_geno + (1 | inoculant) + (1 | supernatant)
# npar     AIC     BIC logLik deviance   Chisq Df Pr(>Chisq)    
# fit2_remstrains_nosupgeno    7 -768.45 -739.50 391.23  -782.45                          
# fit2_remstrain_noint        10 -777.65 -736.29 398.82  -797.65  15.193  3   0.001659 ** 
# fit2_remstrains             19 -934.82 -856.25 486.41  -972.82 175.176  9  < 2.2e-16 ***
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
# bootstrapping of model comparison with interaction but without 168A and 154C strains
# https://jofrhwld.github.io/teaching/courses/2017_lsa/lectures/Session_8.nb.html
# The function you want to apply to each re-fit model. This will almost always be just fixef
# nsim = The number of bootstrap simulations you want to do
...
# PARAMETRIC BOOTSTRAP
# Call:bootMer(x = fit2_remstrains, FUN = fixef, nsim = 1000, verbose = T)
# Bootstrap Statistics :
#     original        bias    std. error
# t1*   0.95297770 -7.385767e-04  0.05007679
# t2*   0.04985044  8.601497e-04  0.04854854
# t3*   0.12175710 -5.976059e-04  0.05449445
# t4*   0.06181804  1.970205e-03  0.04858044
# t5*  -0.04499509 -2.066207e-04  0.04493493
# t6*  -0.37602797 -4.777802e-04  0.05286303
# t7*   0.05627067  5.832990e-04  0.04696957
# t8*   0.07325800  5.180371e-04  0.02672087
# t9*   0.02403195 -3.878089e-04  0.03063555
# t10* -0.03336700 -5.432815e-04  0.02823958
# t11*  0.27146357  5.847295e-04  0.03012319
# t12*  0.32202424  4.063860e-04  0.03554391
# t13*  0.27555200  8.513235e-05  0.03122531
# t14*  0.01389261  3.002062e-04  0.02731726
# t15* -0.04022328  1.510349e-04  0.03109316
# t16* -0.03882403  8.170212e-05  0.02924241
# 
# 44 warning(s): Model failed to converge with max|grad| = 0.00202135 (tol = 0.002, component 1) (and others)
...
boot.ci(fit2_remstrains_bootstraps, index = 3, type = "perc") # inoc_genoOE
# BOOTSTRAP CONFIDENCE INTERVAL CALCULATIONS
# Based on 1000 bootstrap replicates
# CALL :  boot.ci(boot.out = fit2_remstrains_bootstraps, type = "perc", 
#             index = 3)
# Intervals : 
#     Level     Percentile     
# 95%   ( 0.0113,  0.2298 )  
...
boot.ci(fit2_remstrains_bootstraps, index = 6, type = "perc") # sup_genoOE
# BOOTSTRAP CONFIDENCE INTERVAL CALCULATIONS
# Based on 1000 bootstrap replicates
# CALL : boot.ci(boot.out = fit2_remstrains_bootstraps, type = "perc", 
#             index = 6)
...
# do the OC and CC supernatants cause inoculants to grow differently?
# we can see that in I-OA: sup-OC and sup-CC
# in I-OC: sup-OC and sup-CC do not cause difference
# in I-OE: sup-OC and sup-CC do not cause difference
# in I-CC: sup-OC and sup-CC do not cause difference
